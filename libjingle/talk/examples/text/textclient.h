...
#include "talk/examples/text/textsessionclient.h"
...
class TextClient: public sigslot::has_slots<> {
public:
  TextClient(buzz::XmppClient* xmpp_client);
  ~TextClient();

  cricket::PhoneSessionClient* phone_client() const { return phone_client_; }

  void PrintRoster();
  void MakeTextTo(const std::string& name);
...
private:
  ...
  void OnTextCreate(cricket::Text* call);
  void OnTextDestroy(cricket::Text* call);
  const std::string strerror(buzz::XmppEngine::Error err);
  void OnSessionState(cricket::Text* call,
