...
#include "talk/examples/text/textclient.h"
#include "talk/examples/text/console.h"
...
void TextClient::ParseLine(const std::string& line) {
...
TextClient::TextClient(buzz::XmppClient* xmpp_client): xmpp_client_(xmpp_client), roster_(new RosterMap), call_(NULL), incoming_call_(false) {
  xmpp_client_->SignalStateChange.connect(this, &TextClient::OnStateChange);
}
...
const std::string TextClient::strerror(buzz::XmppEngine::Error err) {
  switch (err) {
...
void TextClient::OnTextDestroy(cricket::Text* call) {
  if (call == call_) {
	console_->SetPrompt(NULL);
...
void TextClient::OnJingleInfo(const std::string &relay_token,
...
void TextClient::OnStateChange(buzz::XmppEngine::State state) {
  switch (state) {
  ...
  case buzz::XmppEngine::STATE_OPEN:
    ...
    //InitPhone();
    InitPresence();
    cricket::TextSessionClient *text_client = new cricket::TextSessionClient(xmpp_client_);
    text_client->Start();
    break;
...
void TextClient::InitPhone() {
  ...
  session_manager_->SignalRequestSignaling.connect(
      this, &TextClient::OnRequestSignaling);
  ...
  jit->SignalJingleInfo.connect(this, &TextClient::OnJingleInfo);
  jit->Start();
	  
  phone_client_ = new cricket::PhoneSessionClient(
      xmpp_client_->jid(),session_manager_);
  phone_client_->SignalTextCreate.connect(this, &TextClient::OnTextCreate);

  worker_thread_->Start();
}

void TextClient::OnRequestSignaling() {
  session_manager_->OnSignalingReady();
}

void TextClient::OnTextCreate(cricket::Text* call) {
  call->SignalSessionState.connect(this, &TextClient::OnSessionState);
}

void TextClient::OnSessionState(cricket::Text* call,
...
void TextClient::OnStatusUpdate(const buzz::Status& status) {
  RosterItem item;
  item.jid = status.jid();
...
void TextClient::MakeTextTo(const std::string& name) {
  bool found = false;
  buzz::Jid found_jid;
  ...
  if (found) {
    ...
    phone_client()->SignalTextDestroy.connect(
        this, &TextClient::OnTextDestroy);
    if (!call_) {
      call_ = phone_client()->CreateText();
	  console_->SetPrompt(found_jid.Str().c_str());
      call_->SignalSessionState.connect(this, &TextClient::OnSessionState);
